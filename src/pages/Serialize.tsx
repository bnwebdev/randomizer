...
import { deserialize, getAcceptedFileFormatsByFormat } from "../serialization";
...
const Serialize: FC = () => {
  ...
  const [format, setFormat] = useState<SupportedSerializeFormat>(
    SupportedSerializeFormat.JSON
  );
  ...
  const uploadFileQuery = useCallback(
    async (db: DB, file: File) => {
      ...
      try {
        ...
        const descriptionId = await db.randoms.add({
          randomDescriptions,
          name,
        });
        pushLog(t("serializePage.upload.successLog", { name }), {
          descriptionId,
        });
      } catch (err) {
  ...
  const [uploadFile, { error, loading, called }] =
    useLazyDexie(uploadFileQuery);
    ...
    <>
      ...
      <Form.Group controlId="formFile" className="mb-3">
        ...
        <Form.Select
          ...
          value={format}
          onChange={(e) =>
            setFormat(e.currentTarget.value as SupportedSerializeFormat)
          }
        >
